...
- **auth.js:**
  ...
  - 로그아웃 시 인증 관련 상태 및 SSE 연결을 초기화합니다.
- **notification.js:**
  - 사용자의 실시간 알림 상태 및 읽지 않은 알림 개수를 관리합니다.
  - **SSE 연동:** `connectSSE()`를 통해 백엔드와 연결하고, `EventSource`로부터 `notification` 이벤트를 수신하여 UI를 즉시 갱신합니다.
...
- **자동 토큰 갱신 (Token Refresh):** 
    - 응답 인터셉터가 **401 Unauthorized** 에러를 감지하면, `auth` 스토어의 `refreshToken`을 사용하여 새 `accessToken`을 요청합니다.
    - 토큰 발급에 성공하면 실패했던 원래 요청을 자동으로 재시도합니다.
    - 발급에 실패하거나 `refreshToken`이 만료된 경우 사용자에게 경고 후 로그인 페이지로 리다이렉트합니다.
